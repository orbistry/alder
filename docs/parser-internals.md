...
until M2; the workspace build and CI are red on the branch until then, by design (the parse crates are gated with `-p`).
...
| operators                              | `symbol.rs`                     | **Fixed longest-match table** (custom operators are out of scope): `                                                                                                                                                                                                                                                                                               | > ?? |     | && == != <= >= < > + - * / %`, plus `in`(query mode only). Reserved tokens`| ++ :: .. < | >> << ^`also longest-match and produce`BadOperator`with a hint; `->` separates an arrow-lambda head from its body.`= => += -= *= /=`are **chain terminators** (not consumed). Anything else ends the chain. Longest match means`a==-1`is`a == -1`and`x<-1`is`x < -1`(there is no`<-` token). |
...
argument position and `() -> { x }` / `=> { x }` are blocks returning
...
pub enum BadOperator {
    /// `->` outside a lambda head (hint: `=>` in match arms)
    Arrow,
...
impl<'a> Parser<'a> {
    ...
    /// One nesting level deeper; `too_deep` at the cursor past `MAX_NESTING` (§10.44).
    pub(crate) fn nest<T, E>(&mut self, too_deep: impl FnOnce(Row, Col) -> E, f: impl FnOnce(&mut Self) -> Result<T, E>) -> Result<T, E>;
}
```
...
// expression/lambda.rs  (speculative at an expression start)
...
Raw delimiter errors retain the innermost opener and expected closer from the
scanner's existing stack. `Unbalanced` still reports the unexpected closer's
position; `Endless` retains the outer wrapper position for the parser API, while
the source-aware renderer labels the actual EOF insertion point and the innermost
unclosed delimiter. This diagnostic metadata does not change token acceptance or
the scanner's explicit-stack nesting behavior.
...
**type\_.rs**: var, var_applied (`f[a]`), var_applied_nested (`t[f[a]]`), named_simple, named_qualified, app_one_arg, app_many_args, app_nested, app_result_shorthand, fn_no_params, fn_one_param, fn_many_params, fn_returning_fn, fn_hkt (`fn(a) f[b]`), unit, tuple_pair, tuple_triple, parenthesized, record_empty, record_fields, record_optional_field, record_extension, record_trailing_comma, record_multiline, error_row_empty, error_row_single, error_row_args, error_row_open, error_row_var_only, error_app_unclosed, error_app_empty, error_fn_missing_ret, error_record_missing_colon, error_record_ext_no_fields, error_start, error_reserved.
...
**item/trait\_.rs**: trait_single_fn, trait_hkt, trait_default_body, trait_assoc_type, trait_where, trait_multiple_items (one per line), error_no_params (`trait Show {` → `Trait::Params(TypeParams::Open)`), error_bad_item, error_assoc_type_has_body, error_semicolon_between_items (a one-line `trait Iterator[i] { type Item; fn next(it: i) Option[Item] }` → `Trait::Semicolon`), error_same_line_items (`{ type Item fn next(it: i) Item }` → `Trait::SameLine`).
...
0.6 (withdrawn) A `default-members` narrowing was tried and reverted: the workspace stays whole and is simply red until M2.
...
2. **Fixed operator table with longest match** instead of Elm's maximal munch, so `a==-1` and `x<-1` parse. Reserved tokens (`|`, `++`, `::`, `..`, `<|`, `>>`, `<<`, `^`) produce `BadOperator` hints; `->` is reserved for arrow lambdas. `= => += -= *= /=` terminate a chain and are never operators; `/=` is compound assignment only (the `Stmt::AssignTarget(AssignOp::Div, …)` renderer mentions `!=`).
...
22. **Markup text** follows source-line folding in `docs/markup-whitespace.md`: ordinary multiline prose joins with single spaces after layout trimming; whitespace-only multiline runs disappear; same-line spaces remain. Literal `pre`/`textarea` subtrees preserve text whitespace, with HTML-compatible line-ending normalization. Expression strings are untouched. Text stops at `@` only before `if`/`for`/`match`/`else`/`empty` + non-ident byte, so `a@b.com` is text. Element names accept dashes (custom elements); attribute names accept dashes. Element, attribute and close-tag names are keyword-insensitive (§2.4, §10.36).
...
33. **Workspace** _(internal)_: the workspace is not narrowed; the M1 gate is `cargo clippy -p alder-source -p alder-parse` and `cargo test -p alder-source -p alder-parse`, and the full workspace build and CI are red until M2.
...
40. **Resolved in M2: `provide` is an expression.** `handle` may end its body with `provide Session = session { resolve(event).await }`; `Expr::Provide` becomes the enclosing block's tail and has its body's value. The `no_record_ctor` rule for the `provide … =` head (§2.3) still applies.
...
44. **Nesting limit** _(internal)_. `Parser.depth` counts the recursive entry points — `expression`, `block`, `pattern`, `type_expr`, markup `child`, `style_block` — through `nest(too_deep, f)`; at `MAX_NESTING` (128) the next level is `Expr::TooDeep` / `Block::TooDeep` / `Pattern::TooDeep` / `Type::TooDeep` / `Child::TooDeep` / `Style::TooDeep` at its first byte, nothing consumed, instead of a stack overflow (an uncatchable abort that the fuzz pass reached at ~180–1000 levels on a 2 MB debug thread and ~10 000 in release). A construct may cost more than one level (`<a>{` is a child plus an expression, a nested `query { … ^( … ) }` two expressions), so the practical limit is 64–128 syntactic levels; the constant was chosen so the worst measured construct (`query` pins, ~11 KB of debug stack per syntactic level) stays under 2 MB with a 2× margin. The counter is not part of `ParserState`: `lookahead`/`restore_state` never cross a `nest` boundary.
...
- Lambda and match-arm `{` always being a block means `u -> ({ ..u, name })` needs parentheses; the `Block::LooksLikeRecord` hint covers `{ a: 1 }` but not spread-first records — extend the hint to `..` if it bites.
...
- Markup whitespace normalization is shared parser semantics, not independently repeated by DOM or SSR backends; see `docs/markup-whitespace.md` and formatter equivalence tests.
...
- alder-ast/can/constrain/solve/driver/cli/lsp are red for all of M1; `cargo build --workspace` should be run occasionally to keep the breakage list known. **Known-broken list at the end of M1** (`cargo build --workspace --keep-going`, 2026-09-02): `alder-ast` fails with one error, `E0432: unresolved import alder_source::Docs` (the `Docs` type was dropped in §1; `Associativity` / `Precedence` still resolve). Every other red crate — `alder-can`, `alder-constrain`, `alder-solve`, `alder-driver` (which depend on `alder-ast`), and `alder-cli` / `alder-language-server` behind them — is not even attempted by cargo until `alder-ast` compiles, so their own breakage against the new AST is still unmeasured; M2 starts by fixing `alder-ast`, then re-running the build to get the next list. `alder-region`, `alder-source`, `alder-parse` and `alder-config` are green.
...
- Deep nesting recurses on the Rust stack as the Elm port does today; `MAX_NESTING` (§10.44) turns the overflow into a `TooDeep` error, measured against a 2 MB debug thread. A construct that recurses without passing one of the six counted entry points would escape it; the lib.rs `nesting_limit_*` tests pin the counted ones.
...
Review findings from the end-of-M1 critics (grammar, fuzz, quality) that were not fixed in M1. Each is a small, contained change; none blocks M2 from starting. Fixed in M1 and not listed here: `u32` positions (§10.43), the `@match` close-tag assertion, paren-inclusive regions (§10.43), the nesting limit (§10.44), the missing `while` / `provide` / `assert` / `return` / `break` / `let` error tests and the negative-bigint pattern test, the duplicated `constraint_end`, the `Escape::BadUnicodeLength { code }` → `width` and `WildcardNotVar` `i32` → `usize` renames, the SPEC productions for `error_row`, single-element parentheses, the comma before `..`, and the same-line `(` / `{` of enum variants and constructor patterns, the runtime.md Cloudflare sample (module.rs `docs_runtime_cloudflare`), and the wave-era prose in the test modules.

- **Match arms on one line without a separator.** `match x { 1 => a 2 => b }` parses (expression/match_.rs `match_no_comma_same_line` is a success test) and so does the `@match` form (markup/directive.rs `child_match_arms`); SPEC says arms are separated by `,` or a line break. Either add a `Match::SameLine` / `DirMatch::SameLine` check mirroring `Block::SameLine` (§10.38) and flip that test to an error, or amend SPEC to "a `,` is optional between arms". Decide with the formatter design in M2.
- **`<p>@foo</p>` is `Child::UnknownDirective`, but SPEC's `text` production makes `@foo` text at child start** (it is text mid-run: `a @foo`). The error is the friendlier behaviour for a typo (`@iff`); SPEC's `child` production should gain the rule ("`@` followed by a letter at child start must be one of the five directives"), or `child()` should fall through to `text()` for unknown words.
- **Dangling / lowercase `::` is reported differently per owner.** Types (`Option::foo`) consume the `::` and report `Type::PathMember`; patterns report `Pattern::PathVar` at the path start; `impl Show::show[User]` leaves the `::` and reports `Impl::Open`; `provide Db::`, `where a: Std::`, `<Ui:: />` and `use Db::` pass the same closure for `to_expectation` and `to_member_error` (`Provide::Name`, `Where::Bound`, `Markup::Name`, `Stmt::Use`), so "expected a path" and "dangling `::`" are indistinguishable there. Give those four a `PathMember` twin when the M2 renderer needs to word them apart.
- **`item_end` (item/mod.rs) rescans the source** — it converts the last located node's `Position` back to a byte offset and re-runs `chomp`'s whitespace / `//` logic — because closer bytes (`}`, `)`, trailing `,`) are not recorded in the AST. §10.43 removed the one observable consequence (an item and its statement disagreeing on the end of `let x = (a)`), but the 50 lines of position arithmetic stay; recording the closer position in the sub-parsers would delete them.
- **28 hand-copied comma-list loops** (`grep -rn "Some(b',') =>" crates/alder-parse/src`) with two slightly different trailing-comma shapes (`type_args`, `tag_variant`, `type_fn`, `type_tuple`, `pattern_ctor_args`, `pattern_tuple`, `pattern_array`, expression `tuple_body`, `tag_args`, `array_elements`, `call_args_body`, `record_fields`, `attribute_args`, `import_names`, `type_params`, enum variant args, impl args, `modifier_args`, macro params, query projection). A `comma_list(close, parse_item, to_end_error)` helper on `Parser` would replace them and make the trailing-comma behaviour uniform; positions must not move, so do it under the existing snapshots.
- **`raw_string` / `raw_escape` (raw.rs) duplicate `string_literal`'s byte loop and error-position rules** (EOF → `Endless` at the cursor, newline → `Newline`, escape errors at the backslash). A shared "skip a string literal, no cooking" scanner in string.rs would remove the second copy.
- **`pattern_number` builds a negative literal's text with `format!("-{}", …)` + `alloc_str`** (pattern/mod.rs), a heap round-trip per negative pattern, and re-states the negative-spelling rule that `chomp_number` (number.rs) already implements for `style` dimensions. A `slice_from(start)` gives the same `&'a str` for `Number`; `BigInt` needs the slice minus the trailing `n`.
- **Dead combinators.** `one_of`, `one_of_with_fallback` and `in_context` (lib.rs, the boxed-closure API ported from Elm) have no caller — every alternative is peek-dispatched and every context wrap goes through `specialize` — and `Parser::row()` / `col()` are used only by `test_parser_new`; `position()` and `get_position()` are twin accessors. §1 says the combinators stay; if canonicalization and the formatter still do not use them by the end of M2, delete them and reword §1.
- **`expression/loop_.rs` hosts `loop_`, `state` and `macro_call`** (three unrelated primaries, as §5.13 names the file) and the `macro_call_*` / `error_macro_unbalanced` tests sit in `expression/postfix.rs`. Splitting the file or moving the tests renames snapshots; do it in one commit with `cargo insta test --unreferenced delete`.
- **`space.rs::chomp` is a one-line forward to `eat_spaces`**; fold one into the other when the file is next touched.
- **Docs samples that are fragments, not modules** (found by the fuzz pass, all fail cleanly with in-range positions): language.md Modules (:40), Bindings (:77), Functions (:98), Statements (:133), Pinning (:175), Records (:241), Errors (:327), Typed markup (:454) and Macros (:515, a `...` placeholder inside `comptime { }`); web.md Server hooks (:59), Remote functions (:140), Styles (:199) and Forms (:221) (`{ ... }` placeholder bodies, bare top-level markup); data.md Queries (:53, bare `db.run(…)` statements); runtime.md Cloudflare (:85, a `{ ... }` body). module.rs §7.2 covers each with the fragment wrapped or the placeholder replaced (see the test comments). Not parser defects; the docs owner may want the samples to parse as written, which means wrapping statements in `fn` bodies and dropping `...`.
- **`// OWNER: … (Wave N)` headers** remain on every parser file. They map to §8 and are harmless; remove them when §8 is rewritten for M2.
